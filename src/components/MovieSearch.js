  ...
  return (
    <div className='p-8 bg-black text-white min-h-screen w-screen'>
      <form onSubmit={getMovies} className="mb-6">
        <label htmlFor="movieInput" className='block mb-2 text-lg font-semibold'>Search Movie</label>
        <input
          type="text"
          id="movieInput"
        ...
        />
        <button className='bg-blue-600 px-4 py-2 rounded-lg hover:bg-blue-700'>Search</button>
      </form>

      {error && <p className="error text-red-500 text-lg">{error}</p>}

      <div className="displayMovie grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
        {movieData.map((movie) => (
          <div key={movie.imdbID} className="bg-gray-800 p-3 rounded-lg shadow-md">
            <img src={movie.Poster !== "N/A" ? movie.Poster : "https://via.placeholder.com/150"} alt={movie.Title} className='w-full h-64 object-cover rounded-md' />
